#!/bin/bash
#SBATCH --job-name=hw3    ## Name of the job.
...
pathToOutput="/share/crsp/lab/satwood/llai7/EE283_spring2023/Week_3/DNA_output"

#echo "$pathToOutput/${prefix}"

outputbam=$(basename $prefix)

bwa mem -t 2 -M $ref ${prefix}_1.fq.gz ${prefix}_2.fq.gz | samtools view -bS - > $pathToOutput/${outputbam}.bam
samtools sort $pathToOutput/${outputbam}.bam -o $pathToOutput/${outputbam}.sort.bam

#rm $pathToOutput/${prefix}.bam
